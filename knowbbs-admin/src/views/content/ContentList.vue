<template>
  <div>
    ...
    <el-card>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input placeholder="请输入标题关键词" v-model="query" clearable @clear="getContentList">
        ...
        <el-table-column label="操作" width="130px">
          <template slot-scope='scope'>
            ...
            <el-tooltip effect="dark" content="删除" placement="top" :enterable="false">
              <el-button type="danger" icon="el-icon-delete" size="mini" @click="deleteContentById(scope.row.id)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async getContentList() {
      const { data: res } = await this.$http.get('/content.php')
      this.contentList = res
    },
    // 根据标题搜索帖子
    async queryContent() {
      const { data: res } = await this.$http.get('/content_query.php', { params: { title: this.query } })
      this.contentList = res
    },
    // 展示帖子详情的对话框
