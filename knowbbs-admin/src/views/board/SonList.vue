<template>
  <div>
    ...
    <el-card>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input placeholder="请输入子板块名称" v-model="query" clearable @clear="getSonList">
            <el-button slot="append" icon="el-icon-search" @click="querySon"></el-button>
          </el-input>
        </el-col>
        <el-col :span="4">
      ...
      >
        ...
        <el-table-column label="名称" prop="module_name" width='150' show-overflow-tooltip></el-table-column>
        <el-table-column label="所属父板块" prop="father_module_name" width='150' show-overflow-tooltip></el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 获取子板块列表
    async getSonList() {
      const { data: res } = await this.$http.get('/son.php')
      this.sonList = res
    },
    // 根据名称搜索特定子板块
    async querySon() {
      const { data: res } = await this.$http.get('/son_query.php', { params: { module_name: this.query } })
      this.sonList = res
    },
    // 新增子板块弹窗
    addDialog() {
